...
### CSR 방식에서 SEO 최적화는 어떻게 하나요?

CSR은 브라우저에서 JavaScript를 실행한 뒤 화면을 만들어 보여주는 방식입니다.
이 때문에 검색 엔진이 페이지 내용을 제대로 읽지 못할 수 있어서 SEO에 불리할 수 있습니다.

---

### CSR 환경에서 페이지가 새로고침되면 상태가 초기화되는 이유는?

CSR은 브라우저 메모리 상에서 상태가 유지되므로, 새로고침 시 전체 애플리케이션이 초기화되기 때문입니다.
이를 방지하려면 상태를 localStorage 또는 서버에 저장해야 합니다.

---

### 프론트에서 발생하는 메모리 누수는 어떻게 발생하고, 어떻게 방지할 수 있나요?

이벤트 리스너 해제 누락, 클로저 참조 유지 등으로 누수가 발생하며, useEffect cleanup, WeakMap, Profiler 활용으로 메모리 누수를 방지할 수 있습니다.
